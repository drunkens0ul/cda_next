...
import type { UpdateQuizData, UpdateQuestionData, UpdateAnswerData, QuizWithQuestions } from '@/lib/types/quiz'
...
export default function EditQuizPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const [quizResponse, eventsResponse] = await Promise.all([
          fetch(`/api/admin/quizzes/${slug}`),
        ...
        if (quizData.success && quizData.quiz) {
          const quiz = quizData.quiz as QuizWithQuestions
          setQuiz(quiz)
          ...
          setFormData({
            ...
            descriptionAr: quiz.description.ar,
            eventId: quiz.eventId,
            isActive: quiz.isActive,
            requiresAuth: quiz.requiresAuth,
            ...
            questions: quiz.questions.map(q => ({
              ...
              questionType: q.type as 'multiple_choice' | 'rating' | 'text',
              isRequired: q.required,
              isDeleted: false,
              answers: q.answers.map(a => ({
                id: a.id,
                answerOrder: q.answers.indexOf(a),
                answerText: a.text.en,
                answerTextAr: a.text.ar,
                answerValue: a.value,
                isDeleted: false,
              })),
            })),
  ...
  const addQuestion = () => {
    const newQuestion: UpdateQuestionData = {
  ...
  const addAnswer = (questionIndex: number) => {
    const newAnswer: UpdateAnswerData = {
      ...
      const questions = prev.questions.map((q, qi) => {
        if (qi === questionIndex) {
          const answers = q.answers.map((a, ai) => {
            if (ai === answerIndex && a.id) {
              return { ...a, isDeleted: true }
            }
            return a
          }).filter((a, ai) => ai !== answerIndex || a.id)
  ...
  const validateForm = (): boolean => {
    const activeQuestions = formData.questions.filter(q => !q.isDeleted)
    ...
    for (const q of activeQuestions) {
      if (q.questionType === 'multiple_choice' && q.answers.filter(a => !a.isDeleted).length < 2) {
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-6">
        ...
        <h2 className="text-2xl font-bold text-gray-900">{t('editQuiz')}</h2>
      </div>
            ...
            {formData.questions.length === 0 ? (
              ...
              <div className="space-y-4">
                {formData.questions.map((question, qIndex) => (
                  <div key={question.id || qIndex} className="border rounded-lg p-4 space-y-4">
                    <div className="flex items-center justify-between">
                    ...
                    {question.questionType === 'multiple_choice' && (
                      <div className="space-y-2">
                        ...
                        {question.answers.map((answer, aIndex) => (
                          <div key={answer.id || aIndex} className="flex gap-2">
