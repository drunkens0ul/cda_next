...
function MissionCards() {
  ...
  const cards = [
    {
      icon: <PeopleIcon className="w-[40px] h-[40px]" />,
      title: t('card1Title'),
      description: t('card1Desc'),
    },
    {
      icon: <HandsIcon className="w-[40px] h-[40px]" />,
      title: t('card2Title'),
    ...
    <Section variant="blue" className="pt-12 sm:pt-16 pb-24 sm:pb-32">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
        {cards.map((card, index) => (
          <div key={index} className="bg-vision-card bg-opacity-70 backdrop-blur-md rounded-[3rem] p-10 sm:p-14 hover:bg-opacity-80 transition-all border border-white border-opacity-10 shadow-2xl">
